...
export async function getShiftMetaData(
    ...
    while (true) {
        const url = `https://${instanceUrl}.fs.ocs.oraclecloud.com/rest/ofscMetadata/v1/shifts?offset=${offset}&limit=${limit}`;
...
export function transformData(data: any[]): SheetData {
    ...
    const overview = data.map(d => ({
        ...
        "Type": d.type,
        "WorkTimeStart": d.workTimeStart,
        "WorkTimeEnd": d.workTimeEnd,
    }));

    sheets['Shifts Overview'] = overview;
